...
import { createUser, deleteUser, getUsers, updateUser, type User } from "@/modules/users";
import styles from "./UserCrudPage.module.scss";
      ...
      {!user ? (
        <div className={styles.authGrid}>
          <form className={styles.card} onSubmit={handleRegistration}>
            ...
            <label>Nome<input required minLength={2} value={registration.name} onChange={(event) => setRegistration({ ...registration, name: event.target.value })} /></label>
            <label>E-mail<input required type="email" value={registration.email} onChange={(event) => setRegistration({ ...registration, email: event.target.value })} /></label>
            <label>Senha<input required type="password" minLength={6} value={registration.password} onChange={(event) => setRegistration({ ...registration, password: event.target.value })} /></label>
          ...
          <form className={styles.card} onSubmit={handleLogin}>
            ...
            <label>E-mail<input required type="email" value={credentials.email} onChange={(event) => setCredentials({ ...credentials, email: event.target.value })} /></label>
            <label>Senha<input required type="password" minLength={6} value={credentials.password} onChange={(event) => setCredentials({ ...credentials, password: event.target.value })} /></label>
        ...
        <section className={styles.dashboard}>
          ...
          <form className={styles.editCard} onSubmit={handleUpdate}>
            ...
            <div className={styles.fields}>
              <label>Nome<input required minLength={2} value={editing.name} onChange={(event) => setEditing({ ...editing, name: event.target.value })} /></label>
              <label>E-mail<input required type="email" value={editing.email} onChange={(event) => setEditing({ ...editing, email: event.target.value })} /></label>
              <label>Nova senha <small>(opcional)</small><input type="password" minLength={6} value={editing.password} onChange={(event) => setEditing({ ...editing, password: event.target.value })} /></label>
            </div>
